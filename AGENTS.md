...
This file contains guidance on how to work on kb's own codebase. Guidance on how to drive kb day-to-day lives in the Instruction tree (`kb i show root; kb`) instead. `~/kb/CLAUDE_GLOBAL.md` (= `~/.claude/CLAUDE.md`) contains only the bootstrap that gets that tree read at session start — nothing else.
...
kb is a portable, specialized harness in its own right, not an appendage of Claude Code. Claude Code is one interaction surface that drives kb (today, the primary one) — but kb's own content (the Instruction tree, `kb hooks` detectors, Goals/Todos/Notes) must remain usable from any harness, and kb is the thing responsible for bringing that content along when a new harness is introduced on a machine, not the other way around. Concretely: kb owns *content* (what a mypy failure looks like, what the correct fix is, what a Todo means) as plain data or a plain `kb <verb>` subcommand — read/write text in, text out, no dependency on any one harness's event model or JSON envelope. A harness owns only *mechanism* (Claude Code's `settings.json` hooks, a different tool's equivalent trigger surface) — wiring that is unavoidably harness-specific because only that harness can fire the trigger, kept as thin as possible, doing nothing but extracting that harness's own output into plain text and piping it through the matching `kb <verb>` command. `kb_cli/hooks.py`'s `kb hooks mypy-check` (reads command output on stdin, prints a reminder to stdout if it matches a failed mypy run, silent otherwise) is the model case: the detection logic and reminder text live once, in kb. This is the same content-vs-mechanism split `kb instructions #18` (legacy-claude-code-artifacts) already applies to the Instruction tree; `kb hooks` applies it to hook triggers instead of memory content. When a new kind of "notice X, react with Y" need comes up, the default is a new `kb hooks` subcommand plus a thin per-harness adapter, not a harness-specific script that embeds the logic directly. `docs/harnesses/` holds the actual per-harness wiring instructions (one file per harness, listing every hook set up for it) — read/write there, not here, when setting up or adding a harness adapter; this paragraph is the why, that directory is the how.
...
`Context` is a tree of places/situations, used for two related but distinct purposes: scoping where a `Goal`/`Todo`/`Daily`/`Idea`/`Timer` is actionable (its primary use), and filing anything else -- a `Note`, a `LogEntry` -- that's associated with a place even though it isn't itself a piece of trackable work (e.g. a recipe note filed under a `personal` context, or a log entry about an event that happened at a specific location). Both uses share the one tree and the one `--context` flag; `Tag` is an orthogonal, flat label for "what kind of place this is" (e.g. `tavern`, `player-shop`) that a `Context` can carry several of (`kb context tag NAME TAG...`). A `Goal`/`Todo`/`Daily`/`Idea`/`Timer` is pinned to exactly one of `context_id` or `tag_id`, never both (`HasContextOrTag` mixin, enforced by a validator) — pin it to a `Tag` instead of a `Context` (`--tag` on `todo add`, mutually exclusive with the global `--context`) when the same real-world action is available at every place carrying that tag, e.g. "buy salt" at every tavern, or "buy medium prisms" at every player-shop. A tag-addressed row then prints under *every* Context carrying that tag in `context tree`/`todo tree` output (duplicated on purpose, since it really is actionable in each place), rather than being pinned to one arbitrarily-chosen location.
...
`kb vendor add|list|search`, `kb item add|list|search`, and `kb purchase add|list|search` are the CLI for this area, matching every other entity's `add`/`list`/`search` shape — `kb purchase add VENDOR ITEM` takes existing Vendor/Item names (not raw ids) and looks up or creates the `VendorItem` join row itself. `Vendor`, `Item` (and its `IrlItem`/`PgItem` subtypes), and `Purchase` all compose `HasEmbedding` (`name`+`notes` for Vendor/Item, `notes` for Purchase) the same way `Note`/`Goal`/`Todo` do, so `kb vendor|item|purchase search QUERY` and the top-level `kb search` both cover this area semantically, not just by substring. `VendorItem`/`Purchase` still have no `.create()` classmethod (unlike `Goal`/`Todo`/`Note`) — a script needing to write one directly (rather than through the CLI) uses `kb_repl.py` with a plain constructor, `sess.add(...)`, `sess.flush()` to get the new `.id`, `sess.commit()` at the end. Don't ask before recording a purchase; it's a normal write like any other kb record.
...
See kb Instruction #90 (chrome-devtools MCP vs. Playwright) -- kb's own dev servers: backend on 25690, frontend (Vite) on 25691.
