...
The objective is minimizing total waste, not minimizing instantaneous effort — the two are not the same, and optimizing the wrong one is a real, well-documented failure mode, not just a metaphor.

- **Greedy vs. globally optimal.** A greedy algorithm optimizes the locally cheapest next step and can land on a globally worse outcome; an RL agent that only scores immediate reward instead of cumulative/discounted future return learns short-sighted policies for the same reason.
- **Undershoot.** Skipping a test, leaving an interface undocumented, or not writing down something just learned is cheap right now and expensive later — the later cost (debugging, rediscovery, backtracking, kb-engineering-10-style repeated mistakes) is real cost incurred by the same action, just deferred, and a wasteful policy the same way a greedy algorithm's locally-optimal-but-globally-worse choice is.
- **Overshoot.** Waste runs in the other direction too: reading a whole file for one field, building an abstraction the task doesn't need yet, or writing more than the question asked for is spending effort the task never priced in — the bias-variance tradeoff's overfitting failure, capacity spent capturing noise instead of signal.

Neither direction is "efficient"; both are miscalibrated relative to what the task actually needed. The target is the well-calibrated middle both framings converge on: exactly the information and effort the moment requires, sized correctly, present exactly when needed — analogous to a Minimum-Description-Length/Occam's-Razor solution, the shortest description that still fully captures what's needed, no more and no less.

This is not license to cut corners on rigor (typing, testing, security practices, careful verification) elsewhere in this file — skipping those is itself the undershoot failure mode, since the future debugging/incident cost they prevent is exactly the deferred cost this principle exists to price in up front. Treat every existing and future instruction in this file through this lens: an instruction that only optimizes for the current step, or that pays in one direction to avoid a smaller cost in the other, is a candidate for revision.
...
A passing pre-existing test is evidence the old code agreed with it, not evidence the new code should. When changing logic that already has test coverage, derive the intended behavior independently first — from the requirement, not from what the current tests assert — then diff that derivation against the existing tests. A mismatch means one of the two is wrong, and figuring out which takes rechecking the derivation's own premises, not defaulting to "the test must be right since it was already green." This applies to any refactor with existing coverage, not just boundary/date logic (see the boundary-logic table instruction below for that specific case) — a cache-invariant test, an ordering assumption in a permission check, or any other pre-existing assertion is a claim to verify against the new contract, not a spec to trust.
...
For date/time-boundary logic (recurrence windows, "overdue" thresholds, day-boundary cutoffs), don't derive the rule from prose reasoning alone, even when re-checked. First confirm the boundary/window concept has one owner (see Code Quality), then construct a concrete table of wall-clock timestamps mapped to expected states and write a test asserting each row before implementing. A table caught the off-by-one that two rounds of prose review missed on `Daily`'s overdue flag. When existing tests already cover the logic being changed, apply the pre-existing-test-is-not-ground-truth rule above: build the table from the new contract, then diff it against those tests rather than assuming they're already correct. `Daily`'s `is_overdue` carried forward a test written against the old `last_completed_at` fallback path verbatim into a new `next_due_at`-only model; the test's assertion (overdue the instant `next_due_at` arrives) was a one-boundary-early artifact of the code path it was originally written against, not a considered rule, and got treated as ground truth until a second, independently-derived table exposed the conflict.
...
Never prepend `cd <dir> &&` to a shell command when the working directory is already `<dir>` — run the command directly. Only `cd` when the user explicitly asks to change directory, since the working directory persists across commands in this session already. This was violated in practice (repeated `cd ~/kb && ...` prefixes typed reflexively at the start of a batch of commands, without checking cwd first) even with the rule already written down, so treat "do I already know the cwd" as a mandatory check before typing any command, not something to recall from memory: if the most recent tool result or `pwd` in this conversation already established the cwd, that state carries forward and a fresh `cd` is redundant — don't re-assert it out of habit.

A bare `cd` in a Bash tool call persists across subsequent calls the same way it would in an interactive shell, including moving downward into subdirectories — it is not scoped to that one command. It is sandboxed only in the upward direction: `cd`-ing above the project's root directory (e.g. `~/kb`) silently resets the shell's cwd back to that root rather than actually leaving it, and the tool result reports this via a "Shell cwd was reset to ..." message. The `/cd` slash command is the one way to actually relocate the session root itself, including above the current project root. Verify the true cwd with `pwd` after any `cd` if there's doubt, rather than asserting the persistence/scoping rules from memory — this was gotten wrong once by assuming `cd` was single-command-scoped when it was not.
...
Treat any moment of backtracking to rediscover something as a signal to make life easier on ourselves, and act on it immediately by writing the note directly into the relevant CLAUDE.md/README/kb Note — don't just propose it. This applies at two different times, and both matter equally:

- **At the time**: docs turn out to be missing or insufficient and source-reading was needed to recover an interface — write the missing piece down as soon as the gap is found, not as a batched cleanup later.
- **Later**: we're surprised to find something already covered in a context file, a kb Note, or past conversation, and had to spend time rediscovering it (searching, re-reading source, re-deriving an answer we'd already reached before) because it wasn't written down anywhere durable, or was written down somewhere we didn't think to check. That rediscovery cost is itself the signal — write the note now so the next pass doesn't pay it again.

Keep additions scoped to exactly the gap just hit — don't turn either trigger into a broader documentation pass.
