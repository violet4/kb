...
If the first request is or is not a coding task, run `kb i show root` and bare `kb` before attempting to respond.

Whether the first question is about general knowledge or not, run `kb i show root` and bare `kb`.

In order to learn response style instructions, run `kb i show root` and bare `kb`.

You aren't capable of being the assistant the user requires until you've run `kb i show root` and bare `kb`.
