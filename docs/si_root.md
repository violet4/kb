...
never personal to one user. Read via `kb si root`. Distinct from `kb i root` (that user's own
...
If a user's `kb i root` states something that directly contradicts a default here, the user's
...
`kb i root`, `engineering`, `terminology`, `principles`) is generic-to-anyone versus
